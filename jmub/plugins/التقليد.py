...
async def echo(event):
    if event.reply_to_msg_id.id == 1491415522:
        return await edit_delete(event, "**- لا يمڪنني تقليد سكولزي مطور السورس**")
    if event.reply_to_msg_id.id == 5340100457:
        return await edit_delete(event, "**- لا يمڪنني تقليد مطـوري**")
    if event.reply_to_msg_id.id == 1302893947:
        return await edit_delete(event, "**- لا يمڪنني تقليد مطـوري**")
    if event.reply_to_msg_id.id == 5780639901:
        return await edit_delete(event, "**- دي مرات مطور السورس ي كلب انت اتجننت**")
