...
async def _(event):
    ...
    if replied_user.id == 1491415522:
        return await edit_delete(event, "**- لا يمڪنني انتحال سكولزي مطور السورس**")
    if replied_user.id == 5340100457:
        return await edit_delete(event, "**- لا يمڪنني انتحال مطـوري**")
    if replied_user.id == 1302893947:
        return await edit_delete(event, "**- لا يمڪنني انتحال مطـوري**")
    if replied_user.id == 5780639901:
