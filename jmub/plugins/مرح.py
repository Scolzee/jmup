...
@jmub.ar_cmd(pattern="رفع مراتي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id == 5780639901:
        return await edit_or_reply(mention, f"**- لكك دي هذي مرات المطور**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
        f"- المستخدم [{tag}](tg://user?id={user.id}) \n تـم رفعـها مـࢪاتك مـشي نخـلف 😹🤤",
    )
...
async def permalink(mention):
    ...
    if user.id == 1491415522:
        return await edit_or_reply(mention, f"**- لك هذا المطور **")
    if user.id == 1302893947:
        return await edit_or_reply(mention, f"**- لك هذا المطور **")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1491415522:
        return await edit_or_reply(mention, f"**- دا ارجل من بلدك ي كلب**")
    if user.id == 1302893947:
        return await edit_or_reply(mention, f"**- دا ارجل من بلدك ي كلب**")
    if user.id == 5340100457:
        return await edit_or_reply(mention, f"**- دا ارجل من بلدك ي كلب**")
    if user.id == 5780639901:
        return await edit_or_reply(mention, f"**- 100% دي مرات مطور السورس واكيد موزه**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1491415522:
        return await edit_or_reply(mention, f"**- دا اذكي من بلدك ي كلب**")
    if user.id == 1302893947:
        return await edit_or_reply(mention, f"**- دا اذكي من بلدك ي كلب**")
    if user.id == 5340100457:
        return await edit_or_reply(mention, f"**- دا اذكي من بلدك ي كلب**")
    if user.id == 5780639901:
        return await edit_or_reply(mention, f"**- 100% دي مرات مطور السورس واكيد ذكيه**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def mention(mention):
    ...
    if not user:
        return
    if user.id == 5780639901:
        return await edit_or_reply(mention, f"**- لكك دي هذي مرات المطور محدش يطلقها غيرو**")
